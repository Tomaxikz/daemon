...
PATCHES = {
    ...
    "betterconsole.patch",
    "nsm.patch",
    "nsm-networking.patch",
    "tomaxikz.patch",
}
DOCS = {"README.md", "nsm-networking.manifest.json"}
INSTALLERS = {"betterfiles_patch.sh", "betterconsolepatch.sh", "install_networkstats.sh"}
PAYLOAD = BINARIES | PATCHES | INSTALLERS | DOCS
ASSETS = PAYLOAD | {"RELEASE.json", "SHA256SUMS"}
VERSION_TAG = re.compile(r"v(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-([0-9A-Za-z.-]+))?")
BRANDED_TAG = re.compile(
    r"better-wings-v(0|[1-9]\d*)\.(0|[1-9]\d*)(?:\.(0|[1-9]\d*))?(?:-([0-9A-Za-z.-]+))?"
)
...
def match_tag(tag):
    return VERSION_TAG.fullmatch(tag) or BRANDED_TAG.fullmatch(tag)
...
def version_for(ref, commit):
    ...
    if ref.startswith("refs/tags/"):
        ...
        if not match_tag(tag):
            raise ValueError(
                "Release tags must be vMAJOR.MINOR.PATCH or better-wings-vMAJOR.MINOR[.PATCH], with an optional prerelease suffix"
            )

        return tag if tag.startswith("better-wings-v") else tag[1:]
...
def context(root=ROOT):
    ...
    return {
        ...
        "ref": ref,
        "version": version_for(ref, commit),
        "go_version": "go" + go[1],
...
def verify_elf(path, arch):
    ...
    if (
        ...
        or int.from_bytes(header[18:20], "little") != ARCHES[arch]
    ):
...
def build(directory, arch):
    ...
    (directory / f"build-{arch}.json").write_text(
        json.dumps({**metadata, "files": hashes}, indent=2) + "\n"
    )
...
def bundle(directory, root=ROOT):
    ...
    for name in sorted(PATCHES | INSTALLERS | DOCS):
...
def verify_bundle(directory, expected=None):
    ...
    if expected and any(
        ...
        for key in ("commit", "repository", "version", "go_version", "ref")
    ):
...
class Publisher:
    ...
    def release_by_id(self, release_id):
        ...
        if (
            ...
            or found.get("id") != release_id
            or not isinstance(found.get("assets"), list)
        ):
    ...
    def snapshot(self, tag, metadata, directory):
        ...
        name = tag if BRANDED_TAG.fullmatch(tag) else f"Wings {metadata['version']}"
        ...
        stable = match_tag(tag)
        is_stable = stable is not None and stable[4] is None
        latest = self.api("GET", "releases/latest", missing=True) if is_stable else None
        latest_version = match_tag(latest["tag_name"]) if latest else None
        promote = is_stable and (
            latest is None
            or (
                latest_version is not None
                and tuple(int(part or 0) for part in stable.group(1, 2, 3))
                >= tuple(int(part or 0) for part in latest_version.group(1, 2, 3))
            )
        )
        self.api(
            "PATCH",
        ...
        with tempfile.TemporaryDirectory(prefix="wings-alias-backup-") as temp:
            ...
            try:
                ...
                if previous:
                    try:
                        self.api("PATCH", f"releases/{release['id']}", {"draft": True})
                        self.remove_extra_assets(
                            self.release_by_id(release["id"]),
                            {file.name for file in backup.iterdir()},
                        )
                        if previous["assets"]:
                        ...
                        self.api(
                            "PATCH",
                            f"releases/{release['id']}",
                            {key: previous[key] for key in ("name", "body", "draft", "prerelease")},
                        )
                        print("Publication failed; restored the previous dev-latest release.")
                    except Exception as rollback_error:
                        print(
                            f"Alias recovery failed; use the versioned snapshot and rerun publication: {rollback_error}"
                        )
                raise
...
def publish(directory):
    ...
    if (
        ...
        or metadata["repository"] != "Tomaxikz/daemon"
    ...
    ref = metadata["ref"]
    if ref != "refs/heads/develop" and not (
        ref.startswith("refs/tags/") and match_tag(ref.removeprefix("refs/tags/"))
    ):
