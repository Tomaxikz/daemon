...
func Configure(m *wserver.Manager, client remote.Client) *gin.Engine {
	...
	server.Use(middleware.RequireAuthorization(), middleware.ServerExists())
	{
		...
		files := server.Group("/files")
		{
			...
			files.GET("/nbt", getServerNbtFile)
			files.PUT("/nbt", putServerNbtFile)
			files.GET("/database", getServerDatabaseInspect)
			files.POST("/database/initialize", postServerDatabaseInitialize)
			files.GET("/database/table", getServerDatabaseTable)
			files.POST("/database/table", postServerDatabaseTable)
			files.PATCH("/database/cell", patchServerDatabaseCell)
			files.POST("/database/row", postServerDatabaseRow)
			files.DELETE("/database/row", deleteServerDatabaseRow)
